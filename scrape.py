...
def fetch_page(url, timeout=10):
    """
    Fetch a webpage with basic error handling and logging.
    """
    headers = {
        "User-Agent": (
...
def crawl_site1():
    ...
    if html_content:
        ...
        save_to_csv_site1(table_data, "raw_data_yann_1.csv")
        logging.info("Site1 data extraction completed, saved to raw_data_1.csv")
    else:
        logging.error("Failed to retrieve HTML content for Site1.")
...
def parse_timeline_blocks_from_site2(page_html):
    ...
    try:
        ...
        if not container:
            logging.error("Timeline container not found for Site2. Check the XPath!")
            return []
        blocks = container[0].xpath('.//div[contains(@class, "wphtsp-timeline-block")]')
        if not blocks:
            logging.warning("No timeline blocks found for Site2!")
        return blocks
    except Exception as e:
        logging.error(f"Failed to parse HTML for Site2: {e}")
        return []
...
def parse_block_site2(block):
    ...
    try:
        ...
        if not title_elem:
            logging.warning("Title <a> element not found in a block, skipping for Site2.")
            return None

        title_text = title_elem[0].text_content().strip()
        if ':' not in title_text:
            logging.warning(f"Title format unexpected: {title_text}")
            return None
        ...
        logging.error(f"Error parsing a timeline block for Site2: {e}")
        return None
...
def crawl_site2():
    ...
    if not page_html:
        logging.error("Failed to retrieve HTML content for Site2.")
        return

    blocks = parse_timeline_blocks_from_site2(page_html)
    if not blocks:
        logging.error("No timeline blocks found for Site2. Crawler will stop.")
        return
...
def write_to_csv_site2(data, filename='raw_data_2.csv'):
    ...
    """
    if not data:
        logging.warning("No data to write to CSV for Site2.")
        return
    ...
    try:
        ...
        logging.info(f"Data successfully written to {filename} for Site2.")
    except Exception as e:
        logging.error(f"Failed to write CSV for Site2: {e}")
...
def parse_li_elements_from_site3(page_html):
    ...
    try:
        ...
        if not li_elements:
            logging.warning("No <li> elements found on Site3. Check the XPath or page structure.")
        return li_elements
    except Exception as e:
        logging.error(f"Failed to parse HTML for Site3: {e}")
        return []
...
def parse_item_site3(li_element):
    ...
    try:
        ...
        logging.error(f"Error parsing an item for Site3: {e}")
        return None
...
def crawl_site3():
    ...
    page_html = fetch_page(url)
    if not page_html:
        logging.error("Failed to retrieve HTML content for Site3.")
        return

    li_elements = parse_li_elements_from_site3(page_html)
    if not li_elements:
        logging.error("No <li> elements found. Aborting crawl for Site3.")
        return
...
def write_to_csv_site3(data, filename='raw_data_3.csv'):
    ...
    """
    if not data:
        logging.warning("No data to write to CSV for Site3.")
        return
    ...
    try:
        with open(filename, mode='w', newline='', encoding='utf-8') as csv_file:
            ...
            for item in data:
                writer.writerow(item)
        logging.info(f"Data successfully written to {filename} for Site3.")
    except Exception as e:
        logging.error(f"Failed to write CSV for Site3: {e}")

# -----------------------------------------------------------------------------
# 5. Main Entry Point (Call crawlers as needed)
# -----------------------------------------------------------------------------
if __name__ == "__main__":
    # Uncomment individual crawlers to test them separately.
    logging.info("Starting crawl for Site1...")
    crawl_site1()

    logging.info("Starting crawl for Site2...")
    crawl_site2()

    logging.info("Starting crawl for Site3...")
    crawl_site3()

    logging.info("All crawls completed.")
